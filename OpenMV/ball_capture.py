...
while(True):
    ...
    all_blobs=img.find_blobs([ball],pixels_threshold=400, area_threshold=400, merge=False)
#    print(all_blobs)
    if len(all_blobs)!=0:
        print('in2')
    ...
    #            print("色块的面积为:", area)
                ...
                img.draw_arrow(img_center_x,img_center_y,blob.cx(),blob.cy(),color=(127,255,212),thickness=2)
                print(f"cx:{cx},cy:{cy},b_width:{b_width},b_Height:{b_Height},wh_pro:{wh_proportion}")
                #最后使用的时候一定要删掉\n，这个只是为了输出方便观察！
                #uart_send_dis=f"dist:{b2_dis}"
                #uart_send_string=f"[{str(cx)},{str(cy)},{str(b2_dis)}]\n"
